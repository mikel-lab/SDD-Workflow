...
Specification-Driven Development lifecycle. It coordinates planning, an
...
| Role | Purpose |
| --- | --- |
| `sdd-orchestrator` | Coordinates sources, gates, routing, and review. |
...
The validator checks the exact 24-file distribution, all canonical agent TOMLs,
...
Before every installation, validation runs before any write. A reinstall backs
up every existing managed destination under
`$CODEX_HOME/backups/sdd-workflow-<timestamp>-<process-id>/`. The installer
copies only `skills/sdd-workflow` and the six `sdd-*.toml` files; unmanaged
agents and other Codex files remain untouched. Restore a previous version by
copying its backed-up skill and managed agent files back to the same locations.

## Repository structure

```text
agents/                 Six canonical Codex agent definitions
