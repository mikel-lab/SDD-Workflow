...
4. **Run `speckit-specify`.** Invoke the official skill with `SPECIFY_FEATURE_DIRECTORY` set to the assigned `artifact_directory`. Ground user stories, functional requirements, success criteria, exclusions, assumptions, and its requirements-quality checklist in the analyzed evidence. Record `.specify/feature.json` in `changed_paths` whenever this official action creates or updates it; this is the only permitted pre-gate write outside the assigned directory. Run `validate_cycle.py` with every root-supplied external identity flag, each `--expected-source-id`, and exactly one assigned cycle mode; report its exact command and result before reading generated artifacts.
5. **Run `speckit-clarify` only for unavoidable material blockers.** Apply the materiality categories in [Sources and Artifacts](sources-and-artifacts.md), not SpecKit's broader optional questioning defaults. Exhaust accessible sources, existing user decisions, and safe in-scope recovery first. Return an indispensable question and evidence to the root chat under the lifecycle's User Attention rule; incorporate its relayed answer without asking for a second confirmation. Resolve non-blocking gaps autonomously as documented assumptions.
...
10. **Validate and remediate in a separate Planner action.** Update the manifest `artifacts` inventory, run `validate_cycle.py` with the complete root-supplied external identity and exactly one assigned cycle mode, and report the exact command and result. Route each finding to the owning SpecKit artifact workflow or apply a bounded correction only within the official artifact set. Report changed paths, then rerun `speckit-analyze` read-only.
11. **Repeat analysis and separate remediation** until the package is coherent. At three unsuccessful cycles for the same underlying condition, return diagnosis and attempted recovery to the root chat for internal escalation under [Lifecycle and Gates](lifecycle-and-gates.md). Do not conceal a condition to reach review, repeat an unchanged failing strategy, or convert the retry count into a user-approval gate.
...
Record every non-blocking assumption in the exact four-line format required by [Sources and Artifacts](sources-and-artifacts.md). For an unavoidable material ambiguity, stop the affected planning decision and return the evidence, recovery attempted, impact, and smallest necessary question through the Orchestrator. Incomplete briefs and routine corrections are internal handoffs, not reasons to interrupt the user.

Use the `speckit_root` and assigned `artifact_directory` from the root chat; resolve neither by scanning historical feature packages. After every creation or correction action, inventory all official artifacts actually present, including generated checklist and contract contents, maintain the manifest `artifacts` list, and report the complete set plus exact paths changed in that action. Include `.specify/feature.json` in `changed_paths` and the baseline when the official `speckit-specify` action changed it. Write only within the governed pre-gate boundary; no other `.specify/` path is an exception. Any unauthorized external artifact read invalidates the Planner Result; stop and preserve the evidence without cleanup. Only a concrete question may trigger the bounded read-only lookup in [Remote Memory](remote-memory.md). Report those exact reads separately in `historical_reads`, never in `artifact_reads` or the manifest. No historical lookup selects or continues a cycle; any adopted change follows normal planning review. The Planner does not publish archives or update the remote index.
